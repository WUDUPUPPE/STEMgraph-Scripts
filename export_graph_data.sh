...
for entry in os.listdir(base_dir):                      # Jedes Element im challenges-Ordner anschauen
    ...
    # NEU: author und firstused aus dem Metablock lesen
    author = meta.get("author", "")                     # Wer die Challenge geschrieben hat
    firstused = meta.get("firstused", "")               # Wann sie das erste Mal verwendet wurde
    ...
    if cid not in seen_ids:
        nodes.append({
            ...
            "firstused": firstused                      # NEU: erstes verwendungs-Datum
        })
